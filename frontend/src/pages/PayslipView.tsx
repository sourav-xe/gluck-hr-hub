...
import { fetchAttendanceRecords, fetchLeaveRequests, fetchPayrollRecordById, patchPayrollRecord, convertDocxBlobToPdf } from '@/lib/hrApi';
...
export default function PayslipView() {
  ...
  const autoState = (location.state || {}) as
    | { autoDownload?: boolean; templateId?: string; startInEdit?: boolean; downloadFormat?: 'docx' | 'pdf' }
    | undefined;
  ...
  const [payslipFormat, setPayslipFormat] = useState<'docx' | 'pdf'>(() =>
    autoState?.downloadFormat === 'pdf' ? 'pdf' : 'docx'
  );
  ...
  const generateAndDownload = async (templateIdOverride?: string) => {
    ...
    try {
      ...
      let { blobUrl } = await generatePayslipFromTemplate({
      ...
      if (payslipFormat === 'pdf') {
        const docxBlob = await fetch(blobUrl).then((r) => r.blob());
        URL.revokeObjectURL(blobUrl);
        const pdfRes = await convertDocxBlobToPdf(docxBlob);
        if ('error' in pdfRes) throw new Error(pdfRes.error);
        blobUrl = URL.createObjectURL(pdfRes);
        downloadName = filename.replace(/\.docx$/i, '.pdf');
      }

      setProgress(100);
      stopProgress();
      downloadBlob(blobUrl, downloadName);
      toast({ title: 'Payslip downloaded', description: downloadName });
    } catch (e: unknown) {
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 justify-end">
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground whitespace-nowrap hidden sm:inline">Format</span>
            <Select value={payslipFormat} onValueChange={(v) => setPayslipFormat(v as 'docx' | 'pdf')}>
              <SelectTrigger className="rounded-xl h-9 w-[118px] text-xs" disabled={generating || saving}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
      ...
      <div className="glass-card rounded-2xl p-8 print:shadow-none">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm mb-6 glass-card rounded-xl p-4">
          ...
          <div>
            ...
            <span className="font-mono">{emp.employeeCode || emp.id}</span>
          </div>
        </div>
