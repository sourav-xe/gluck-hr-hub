...
import { generateSimpleTemplateDocument } from '@/lib/hrApi';
import { writeEmailDraftStash } from '@/lib/emailDraftStash';
import { formLabelForSimpleField, inputPlaceholderForFormLabel } from '@/lib/simpleTemplateFieldLabel';
...
export default function GenerateFromTemplate() {
  ...
  const hideProgressSoon = () => {
    setTimeout(() => setGenerationProgress(null), 900);
  };
  ...
  const handleGenerateDownload = async () => {
    ...
    try {
      const baseName = (documentName || 'generated').replace(/\.(docx|pdf)$/i, '');
      const ext = outputFormat === 'pdf' ? 'pdf' : 'docx';
      const blob = await generateSimpleTemplateDocument(template.id, fieldValues, outputFormat, (p) =>
        setGenerationProgress(p)
      );
      if ('error' in blob) throw new Error(blob.error);
      triggerBrowserDownload(blob, `${baseName}.${ext}`);
      toast({ title: 'Document ready', description: 'Download started.' });
  ...
  /** Saves the same file for Announcements → Email (“Draft files”) and downloads it. */
  const handleDraftDownload = async () => {
    ...
    try {
      ...
      if ('error' in blob) throw new Error(blob.error);
      const fileName = `${baseName}-draft.${ext}`;
      await writeEmailDraftStash(blob, fileName);
      triggerBrowserDownload(blob, fileName);
      toast({
        title: 'Draft ready',
        description: 'Saved for email. Use Announcements → Draft files to attach.',
      });
      setGenerationProgress(100);
    } catch (err: unknown) {
      toast({ title: 'Draft failed', description: (err as Error).message, variant: 'destructive' });
    } finally {
      setGenerating(false);
      hideProgressSoon();
    }
        ...
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 w-[min(720px,92vw)]">
          <div className="glass-card rounded-2xl p-4 border border-border/50">
            ...
            <p className="text-[10px] text-muted-foreground mt-2">
              Runs on the server. The bar may climb slowly while the PDF is built (no data is downloaded until then). Safe to
              wait — this tab should stay responsive.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="glass-card rounded-2xl p-6 space-y-6 lg:rounded-r-none lg:border-r-0 flex flex-col lg:h-full">
          <div>
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key !== 'Enter' || !template.fields[0]) return;
                e.preventDefault();
                const ph = template.fields[0].placeholder;
                fieldInputRefs.current[ph]?.focus();
          ...
          <div className="space-y-4 flex-1 overflow-auto pr-1">
            <div className="flex flex-col gap-0.5 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
              ...
              <p className="text-[10px] text-muted-foreground">Press Enter to jump to the next field</p>
            </div>

            {template.fields.map((field, i) => (
              <div key={field.placeholder}>
                <Label className="text-xs text-muted-foreground">{formLabelForSimpleField(field)}</Label>
                <Input
                  ref={(el) => {
                    fieldInputRefs.current[field.placeholder] = el;
                  }}
                  value={fieldValues[field.placeholder] || ''}
                  onChange={(e) => setFieldValues((prev) => ({ ...prev, [field.placeholder]: e.target.value }))}
                  placeholder={inputPlaceholderForFormLabel(formLabelForSimpleField(field))}
                  ...
                  onKeyDown={(e) => {
                    ...
                    if (next) {
                      fieldInputRefs.current[next.placeholder]?.focus();
                      setActivePlaceholder(next.placeholder);
                    }
                  }}
                />
              </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={outputFormat} onValueChange={(v) => setOutputFormat(v as 'docx' | 'pdf')}>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => void handleDraftDownload()}
            ...
            <Button
              type="button"
              onClick={() => void handleGenerateDownload()}
              disabled={generating || template.fields.length === 0}
              className="flex-1 rounded-xl h-11 gap-2 shadow-md shadow-primary/20"
            >
              {generating ? (
