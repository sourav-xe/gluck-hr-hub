import { Employee, UserRole } from '@/types/hr';
...
export async function updateEmployee(
  ...
  payload: Partial<Employee> & {
    password?: string;
    app_role?: UserRole;
    /** `null` clears custom list (role default). */
    sidebarNavAllow?: string[] | null;
  }
): Promise<{ ok: true; employee: Employee } | { ok: false; error: string }> {
  ...
  if (payload.app_role !== undefined) body.app_role = payload.app_role;
  if (payload.sidebarNavAllow !== undefined) body.sidebarNavAllow = payload.sidebarNavAllow;
