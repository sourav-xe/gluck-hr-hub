  ...
  return (
    <div className="animate-fade-in max-w-3xl">
      <PageHeader
        ...
        action={<Button variant="ghost" onClick={() => navigate('/documents')} className="gap-2 rounded-xl"><ArrowLeft className="w-4 h-4" /> Back</Button>}
      />
        ...
        {[1, 2, 3, 4].map(s => (
          <div key={s} className="flex items-center gap-2">
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-xs font-bold transition-all ${
              ...
              s === step ? 'bg-primary text-primary-foreground shadow-md shadow-primary/20' :
        ...
        {step === 1 && (
          <div className="space-y-5">
            ...
            <Select value={docType} onValueChange={v => setDocType(v as DocumentType)}>
              <SelectTrigger className="rounded-xl h-10"><SelectValue placeholder="Choose document type" /></SelectTrigger>
              <SelectContent>
                {docTypes.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
              </SelectContent>
            </Select>
            <Button disabled={!docType} onClick={() => setStep(2)} className="rounded-xl">Next</Button>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <Select value={linkedEmployee} onValueChange={setLinkedEmployee}>
              <SelectTrigger className="rounded-xl h-10"><SelectValue placeholder="Select employee" /></SelectTrigger>
              <SelectContent>
            ...
            <div className="flex gap-2">
              ...
              <Button disabled={!linkedEmployee} onClick={handleGenerate} className="rounded-xl">Next</Button>
            </div>
          </div>
        )}
        ...
        {step === 3 && docType && (
          <div className="space-y-5">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => setStep(4)} className="rounded-xl">Preview</Button>
            </div>
          </div>
        )}
          ...
          <div className="space-y-5">
            ...
            <div className="border border-border/50 rounded-xl p-6 bg-background min-h-[300px]">
              ...
              <div className="space-y-2 text-sm">
                <p>Date: <span className="font-mono">{fields['Document Date'] || new Date().toLocaleDateString()}</span></p>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" className="gap-2 rounded-xl" onClick={() => toast({ title: '📥 Downloaded', description: 'Document downloaded as DOCX' })}>
                <Download className="w-4 h-4" /> Download DOCX
              </Button>
              <Button variant="outline" className="gap-2 rounded-xl" onClick={() => toast({ title: '📥 Downloaded', description: 'Document downloaded as PDF' })}>
                <Download className="w-4 h-4" /> Download PDF
              </Button>
              <Button className="gap-2 rounded-xl shadow-md shadow-primary/20" onClick={() => { toast({ title: '✅ Saved', description: 'Document saved to employee record' }); navigate('/documents'); }}>
