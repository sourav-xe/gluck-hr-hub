...
export default function AttendancePage() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Attendance"
        description="Monthly attendance overview with clock-in/out data"
        action={<Button variant="outline" className="gap-2 rounded-xl" onClick={handleExport}><Download className="w-4 h-4" /> Export CSV</Button>}
      />
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-border/50">
              <th className="text-left p-3 sticky left-0 bg-card dark:bg-card min-w-[140px] font-semibold text-muted-foreground text-[11px] uppercase tracking-wider z-10">Employee</th>
                    ...
                    <td key={day} className={`p-1 text-center ${weekend ? 'bg-muted/30' : ''}`}>
                      {weekend ? (
                        ...
                        <div className="group relative">
                          <span className={`inline-flex items-center justify-center w-6 h-6 rounded-md text-[9px] font-bold cursor-default ${statusColors[record.status]}`}>
                            {record.status}
                          </span>
                          {(record.clockIn || record.clockOut) && (
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1 hidden group-hover:block z-20">
                              <div className="glass-card rounded-lg p-2 text-[9px] whitespace-nowrap shadow-lg">
                                {record.clockIn && <div className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" /> In: {record.clockIn}</div>}
                                {record.clockOut && <div className="flex items-center gap-1"><Clock className="w-2.5 h-2.5" /> Out: {record.clockOut}</div>}
                                {record.totalHours && <div className="font-semibold mt-0.5">{record.totalHours}h total</div>}
                              </div>
                            </div>
                          )}
                        </div>
