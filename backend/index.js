...
import { User, Employee, Counter, AnnouncementSettings, SimpleDocTemplate, Festival } from './models.js';
import { analyzeDocxPlaceholders } from './services/docxRedAndMustache.js';
import { suggestPlaceholdersWithAi, inferSnippetLabel } from './services/aiPlaceholders.js';
import { docxBufferToPdf } from './services/docxToPdf.js';
import { mergeSimpleDocxTemplate } from './services/simpleDocxMerge.js';
import { registerHrDataRoutes } from './routes/hrData.js';
import { registerDocumentAutomationRoutes } from './routes/documentAutomation.js';
...
async function allocateNextEmployeeCode() {
  ...
  const n = doc.seq;
  return `GG${String(n).padStart(3, '0')}`;
}

const ALLOWED_SIDEBAR_NAV_KEYS = new Set([
  'dashboard',
  'employees',
  'my-profile',
  'attendance',
...
function serializeEmployee(doc) {
  ...
  return {
    id: o._id.toString(),
    employeeCode: o.employeeCode ? String(o.employeeCode) : undefined,
    fullName: o.fullName,
    email: o.email,
    phone: o.phone || '',
    type: o.type,
    ...
    sidebarNavAllow:
      Array.isArray(o.sidebarNavAllow) && o.sidebarNavAllow.length > 0 ? [...o.sidebarNavAllow] : null,
  };
}
...
function userResponse(userDoc, employeeDoc) {
  ...
  const sidebarNavAllow =
    employeeDoc &&
    Array.isArray(employeeDoc.sidebarNavAllow) &&
    employeeDoc.sidebarNavAllow.length > 0
      ? [...employeeDoc.sidebarNavAllow]
      : undefined;
  ...
  return {
    ...
    employeeCode: employeeDoc?.employeeCode ? String(employeeDoc.employeeCode) : undefined,
    onboardingComplete,
    needsOnboarding,
    sidebarNavAllow,
  };
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    let empDoc = null;
    if (user.employeeId) {
      empDoc = await Employee.findById(user.employeeId)
        .select('onboardingComplete onboardingStep phone address employeeCode sidebarNavAllow')
        .lean();
    }
...
app.get('/api/employees/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const payload = serializeEmployee(emp);
    if (emp.userId) {
      const u = await User.findById(emp.userId).select('app_role').lean();
      if (u?.app_role) payload.appRole = normalizeRole(u.app_role);
    }
    res.json(payload);
  } catch (e) {
    console.error(e);
...
app.patch('/api/employees/:id', authMiddleware, async (req, res) => {
  try {
    ...
    const authActor = await User.findById(req.auth.userId).lean();
    const actorIsSuper = authActor?.app_role === 'super_admin';
    ...
    if (b.sidebarNavAllow !== undefined) {
      if (!actorIsSuper) {
        return res.status(403).json({ error: 'Only Super Admin can manage sidebar access' });
      }
      if (b.sidebarNavAllow === null) {
        emp.set('sidebarNavAllow', undefined);
      } else if (Array.isArray(b.sidebarNavAllow)) {
        const filtered = [
          ...new Set(
            b.sidebarNavAllow
              .map((k) => String(k || '').trim())
              .filter((k) => ALLOWED_SIDEBAR_NAV_KEYS.has(k))
          ),
        ];
        if (!filtered.includes('dashboard')) filtered.unshift('dashboard');
        emp.sidebarNavAllow = filtered;
      }
    }

    if (b.app_role !== undefined && actorIsSuper && emp.userId) {
      const nr = normalizeRole(b.app_role);
      const roleList = ['super_admin', 'hr_manager', 'reporting_manager', 'employee', 'freelancer_intern'];
      if (roleList.includes(nr)) {
        await User.updateOne({ _id: emp.userId }, { $set: { app_role: nr } });
      }
    }
    ...
    const out = serializeEmployee(emp);
    if (emp.userId) {
      const u = await User.findById(emp.userId).select('app_role').lean();
      if (u?.app_role) out.appRole = normalizeRole(u.app_role);
    }
    res.json(out);
  } catch (e) {
    console.error(e);
...
registerEmailRoutes(app, authMiddleware);
...
app.post('/api/doc-simple-templates', authMiddleware, memUpload.single('file'), async (req, res) => {
  try {
    ...
    // Parse red-colored text fields from DOCX and assign stable keys once at upload time.
    let fields = [];
    try {
      const { redSnippets } = analyzeDocxPlaceholders(file.buffer);
      const aiOrHeuristic = await suggestPlaceholdersWithAi(redSnippets || []);
      fields = aiOrHeuristic
        .map((item, i) => {
          const placeholder = String(item?.exampleValue || '').trim() || String(redSnippets?.[i] || '').trim();
          const labelRaw = String(item?.label || '').trim();
          return {
            fieldName: String(item?.key || '').trim() || `field_${i + 1}`,
            placeholder,
            label: labelRaw || inferSnippetLabel(placeholder),
          };
        })
...
function sanitizeSimpleTemplateFieldValues(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return {};
  const out = {};
  for (const [k, v] of Object.entries(raw)) {
    if (typeof k !== 'string' || k.length > 2048) continue;
    if (typeof v !== 'string') continue;
    out[k] = v.slice(0, 16000);
  }
  return out;
}

/** Merge red placeholders + optional PDF on the server (keeps browser UI responsive). */
app.post('/api/doc-simple-templates/:id/generate', authMiddleware, async (req, res) => {
  try {
    const fieldValues = sanitizeSimpleTemplateFieldValues(req.body?.fieldValues);
    const format = req.body?.format === 'pdf' ? 'pdf' : 'docx';

    const doc = await SimpleDocTemplate.findById(req.params.id).lean();
    if (!doc?.originalFileBase64) return res.status(404).json({ error: 'Template not found' });

    const buf = Buffer.from(doc.originalFileBase64, 'base64');
    const merged = await mergeSimpleDocxTemplate(buf, fieldValues);
    ...
    console.error('[doc-simple-templates/generate]', e?.message || e);
    res.status(500).json({ error: e.message || 'Generation failed' });
  }
});

app.post('/api/convert-docx-to-pdf', authMiddleware, memUpload.single('file'), async (req, res) => {
  try {
    const file = req.file;
    if (!file?.buffer) return res.status(400).json({ error: 'No DOCX file uploaded' });
    const pdfBuf = await docxBufferToPdf(file.buffer);
    res.setHeader('Content-Type', 'application/pdf');
    res.send(pdfBuf);
  } catch (e) {
    res.status(500).json({ error: e.message || 'PDF conversion failed' });
  }
});
// ────────────────────────────────────────────────────────────────────────────

async function backfillEmployeeCodesIfRequested() {
  if (process.env.BACKFILL_EMPLOYEE_CODES !== '1') return;
  const missing = await Employee.find({
    ...
    .lean();
  if (!missing.length) {
    console.log('[HR API] BACKFILL_EMPLOYEE_CODES: no employees without a code');
    return;
  }
  ...
  for (const row of missing) {
    ...
    await Employee.updateOne({ _id: row._id }, { $set: { employeeCode: code } });
  }
  await Counter.findOneAndUpdate({ _id: 'employeeCode' }, { $set: { seq } }, { upsert: true });
  console.log('[HR API] BACKFILL_EMPLOYEE_CODES: assigned', missing.length, 'code(s); counter =', seq);
}
...
async function start() {
  ...
  await backfillEmployeeCodesIfRequested();

  app.listen(PORT, () => {
    console.log('[HR API] Server started on port', PORT);
    console.log('[HR API] Local URL    = http://localhost:' + PORT);
